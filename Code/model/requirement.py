    ...
    def all(self, project):
        ...
        if user:
            ...
            if len(requirements) == 0 and project_obj.title == "Toolbox":
                ...
                requirement.put()
                requirement = Requirement(parent = project, author = user, id = "2", title = 'In-App Chat', content = 'Eine Liste soll alle Teammitglieder mit Online Status anzeigen. Bei Klick auf den Namen eines Teammitglieds, das online ist, soll ein Chatfenster aufgehen.')
                requirement.put()
                requirements = query.fetch()
